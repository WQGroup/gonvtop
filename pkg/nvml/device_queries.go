...
// DeviceGetProcessUtilization nvmlDeviceGetProcessUtilization
func (a API) DeviceGetProcessUtilization(device Device, lastSeenTimeStamp uint64) ([]ProcessUtilizationSample, error) {

	var ProcessSamplesCount uint32
	var err error
	err = a.call(a.nvmlDeviceGetProcessUtilization, uintptr(device), uintptr(0), uintptr(unsafe.Pointer(&ProcessSamplesCount)), uintptr(unsafe.Pointer(&lastSeenTimeStamp)))
	
	if ProcessSamplesCount == 0 {
		return []ProcessUtilizationSample{}, err
	}
	utilization := make([]ProcessUtilizationSample, ProcessSamplesCount)
	err = a.call(a.nvmlDeviceGetProcessUtilization, uintptr(device), uintptr(unsafe.Pointer(&utilization[0])), uintptr(unsafe.Pointer(&ProcessSamplesCount)), uintptr(unsafe.Pointer(&lastSeenTimeStamp)))
	return utilization[:ProcessSamplesCount], err
}
