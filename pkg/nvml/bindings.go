...
type API struct {
	...
	nvmlDeviceGetProcessUtilization *syscall.Proc
}
...
// Shutdown shutdowns NVML by releasing all GPU resources previously allocated with Init() and
...
func New(path string) (*API, error) {
	if path == "" {
		// TODO: 后期需要适配 Linux，有待测试
		// 判断是什么操作系统, Windows 还是 Linux
		if os.PathSeparator == '\\' {
			path = "nvml.dll" // C:\Windows\System32\nvml.dll
		} else {
			path = "libnvidia-ml.so.1" // /usr/lib/x86_64-linux-gnu/libnvidia-ml.so.1
		}
	}
	...
	bindings := &API{
		...
		nvmlDeviceGetProcessUtilization:              dll.MustFindProc("nvmlDeviceGetProcessUtilization"),
	}
